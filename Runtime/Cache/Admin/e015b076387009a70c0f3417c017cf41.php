...
<body>
    ...
    <div class="cf">
        <button class="btn ajax-post confirm" url="<?php echo U('sponsorDel');?>" target-form="ids">删 除</button>
    ...
    <div class="data-table table-striped">
        <table>
            ...
            <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php if($vo['status'] == 1): ?>未处理
                                <?php else: ?>
                                已处理<?php endif; ?>
                        </td>
                        <td>
                            <a title="详情" href="<?php echo U('sponsorEdit',array('id'=>$vo['id']));?>">详情</a>
                            <?php if($vo['status'] == 1): ?><a title="设为已处理" href="<?php echo U('changeSponApply',array('id'=>$vo['id'],'value'=>2));?>">设为已处理</a>
                                <?php else: ?>
                                <a title="设为未处理" href="<?php echo U('changeSponApply',array('id'=>$vo['id'],'value'=>1));?>">设为未处理</a><?php endif; ?>

                            <a class="confirm ajax-get" title="删除" href="<?php echo U('sponsorDel',array('id'=>$vo['id']));?>">删除</a>
                        </td>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#status').change(function(){
                ...
                window.location.href = '<?php echo U("sponsorApply");?>' + '&status=' + thisVal
            })
        });
    </script>
    <script type="text/javascript">
